...
import models as M
from LoadData import chunk_data, load_document
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        # 选择向量数据库
        embedding = st.selectbox("请选择向量数据库", ("FAISS", "Chroma"))
        ...
        # 检索数量设置
        k = st.number_input(
            "top_k", min_value=1, max_value=20, value=3, on_change=clear_history
        )
        ...
        if uploaded_file and add_data:  # 如果用户上传了文件
            with st.spinner("正在读取、分割和嵌入文件..."):
                ...
                st.write(
                    f"Chunk大小: {chunk_size}, 重叠度: {chunk_overlap}, 总片段数: {len(chunks)}"
                )
